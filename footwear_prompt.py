...
genai.configure(api_key="AIzaSyBiwRcplrq_XVZ3kU8e468awlbbEMkxIhs")
...
def get_image_prompt_from_gemini(image_path):
    ...
    try:
        ...
        # Create the prompt
        prompt_text = """Describe this image in high detail to be able to re-generate a similar image with the same background and aesthetics with any other item, (make the prompt more defining for the image, and ignore any texts or logo from it). 
        Focus on:
...
def main():
    # Define the folders and their order for footwear
    folders = [
        ("footwear/casual-shoes", "casual shoes"),
        ("footwear/formal-shoes", "formal shoes"),
        ("footwear/Model-casual-shoes", "model with casual shoes"),
        ("footwear/Model-formal-shoes", "model with formal shoes"),
        ("footwear/Model-sports-shoes", "model with sports shoes")
    ]
    ...
    print(f"Found {len(image_paths)} footwear images to process")
    print("=" * 50)
    ...
    txt_filename = "footwear_prompt_output.txt"
    
    with open(txt_filename, 'w', encoding='utf-8') as txtfile:
        for result in results:
            txtfile.write(f"PROMPT {result['Image_Number']}:\n")
            txtfile.write(f"Image: {result['Filename']}\n")
            txtfile.write(f"Category: {result['Folder_Category']}\n")
            txtfile.write(result['Generated_Prompt'])
            txtfile.write("\n\n")
    
    print(f"\nResults saved to {txt_filename}")
    print(f"Successfully processed {len(results)} footwear images")
    
    # Print summary
    print("\nSummary of processed footwear images:")
    for result in results:
